# PattaPe API Contract: `POST /predict`
...
| `disease` | `string` | ML | Predicted disease class identifier (snake_case matching `classes.json`). Primary source of truth. |
| `disease_label_i18n` | `object` (`dict[string, string]`) | Backend | Localized disease display names for farmer UI (keys: `en`, `hi`, `ta`). |
| `confidence` | `number` (`float`, 0.0–1.0) | ML | Softmax probability score of the top predicted class from EfficientNet-B0. || `top3` | `array[object]` | ML | Top 3 ranked candidate predictions. Each item contains `{"disease": string, "confidence": float}`. |
...
| `gemini` | `object` / `null` | Backend / ML | Multimodal second-opinion & explanation generated by Gemini 2.5 Flash-Lite (`ml/gemini_explainer.py`). Null if offline or unconfigured. |
| `gemini.gemini_assessment` | `string` | Backend / ML | Disease independently assessed by Gemini (must match allowed crop diseases or `"uncertain"`). |
| `gemini.agreement` | `boolean` | Backend / ML | `true` if Gemini's assessment matches the EfficientNet primary diagnosis, `false` if they disagree. |
| `gemini.assessment_confidence` | `string` | Backend / ML | Gemini confidence level: `"high"`, `"medium"`, `"low"`, or `"uncertain"`. |
| `gemini.visual_evidence` | `array[string]` | Backend / ML | 2-4 factual visual observations from the image supporting the diagnosis. |
| `gemini.possible_causes` | `array[string]` | Backend / ML | 1-3 plausible high-level agronomic/pathological causes. |
| `gemini.farmer_explanation` | `string` | Backend / ML | Plain-language explanation (2-4 sentences) tailored for an Indian farmer without technical jargon. |
| `gemini.disagreement_reason` | `string` / `null` | Backend / ML | Explanation if Gemini and EfficientNet disagree (`null` when `agreement` is `true`). || `escalate` | `boolean` | Backend | Whether this case must escalate to a human agricultural extension officer. |
...
The decision to escalate a case to an agricultural officer is deterministic and implements the following condition:

```text
escalate = (severity IN ["moderate", "severe"]) 
        OR (risk_72h.level == "high") 
        OR (confidence < 0.70)
        OR (gemini != null AND gemini.agreement == false AND gemini.assessment_confidence IN ["high", "medium"])```
...
4. **AI Disagreement (Gemini ≠ EfficientNet):** When the multimodal second opinion strongly disputes the primary classifier, human officer validation is triggered (`escalate_reason: "model_and_gemini_disagreement"`).
---
...
|    - Core Vision: EfficientNet-B0 crop-aware diagnosis (classes.json)             |
|    - Explainability: Grad-CAM heatmap overlay + affected_pct                      |
|    - Second Opinion (Optional): Gemini 2.5 Flash-Lite (gemini_explainer.py)       |+-----------------------------------------+-----------------------------------------+
                                          |
                                          v
+-----------------------------------------+-----------------------------------------+
| 2. Backend & Rule Engine (B1, C1)                                                 |
|    - Ingests ML inference output (predict.py + gradcam.py)                        |
|    - Calls Gemini second opinion (gemini_explainer.py) if GEMINI_API_KEY present  |
|    - Computes deterministic severity tier (ml/severity.py) from affected_pct      |
|    - Fetches agromet weather & evaluates deterministic 72h risk (ml/risk.py)      |
|    - Attaches i18n labels & IPM advisory templates (T1-T8)                        |
|    - Evaluates escalation rule (including Gemini conflict) and assigns case_id   ||    - Exposes POST /predict conforming to this contract                           |
+-----------------------------------------+-----------------------------------------+
                                          |
                                          v
+-----------------------------------------+-----------------------------------------+
...
|    - F1: Displays heatmap, severity, risk badge, advisory blocks, AND            |
|          Gemini Second Opinion card (agreement pill, symptoms, farmer summary)   |
|    - F1: Voice readout button synthesizes gemini.farmer_explanation or advisory   |
|    - F2: Officer dashboard highlights cases with model-Gemini disagreements       |
|    - F2: Manages officer validation & case queue                                  |+-----------------------------------------------------------------------------------+
```

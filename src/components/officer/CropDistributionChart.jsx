...
// ─── Custom X-Axis Tick (emoji + name) ───────────────────────────────────────

function CropTick({ x, y, payload }) {
  const meta = CROP_META[payload.value] || {};
  return (
    <g transform={`translate(${x},${y})`}>
      <text x={0} y={0} dy={4} textAnchor="middle" fontSize={16}>
        {meta.icon || '🌱'}
      </text>
 ...
 *
 * Props:
 *  cropStats    – stats.by_crop object from getStats(); primary data source
 *  selectedCrop – currently active crop filter string ('all' | crop name)
 *  onCropSelect – callback(crop) to set crop filter in parent
 */
export default function CropDistributionChart({ cropStats, onCropSelect, selectedCrop = 'all' }) {
  // Derive chart data dynamically from seed stats — no hardcoded numbers
  const chartData = CROP_ORDER.map((crop) => {
    const s = cropStats?.[crop] || { healthy: 0, at_risk: 0, infected: 0 };
    return {
      name: crop,
      Healthy:   s.healthy,
      'At Risk': s.at_risk,
  ...
  const totalCases = chartData.reduce(
    (s, d) => s + d.Healthy + d['At Risk'] + d.Infected,
    0
  );

  const handleBarClick = (data) => {
    if (data?.activeLabel && onCropSelect) {
    ...
    <div className="bg-white rounded-3xl border-2 border-slate-300 shadow-xl p-5 sm:p-6 flex flex-col gap-4">
      ...
      <div style={{ width: '100%', height: 268 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart
            data={chartData}
            margin={{ top: 6, right: 8, left: -22, bottom: 28 }}
            barCategoryGap="28%"
